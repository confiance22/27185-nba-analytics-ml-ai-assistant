...
Loads cleaned tables from Snowflake, prints descriptive statistics,
produces 3 charts, and prints a plain-language summary.
...
def print_dataset_overview(teams, games):
    print("=" * 70)
    ...
    for name, df in [("cleaned_teams", teams), ("cleaned_games", games)]:
        ...
        print(f"\nMissing values:\n{pd.DataFrame({'count': missing, 'percent': missing_pct}).to_string()}")
...
def print_univariate(games, teams):
    print("\n" + "=" * 70)
...
def print_bivariate(games, joined):
    print("\n" + "=" * 70)
    print("3. BIVARIATE ANALYSIS")
    print("=" * 70)
    print("\n--- Average combined score by season ---")
...
def generate_charts(games, joined, teams):
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    conf_rates = joined.groupby("home_conference")["home_win"].mean().mul(100).round(1)
...
def print_summary(teams, games, joined):
    ...
    east_rate = round(joined[joined["home_conference"] == "East"]["home_win"].mean() * 100, 1)
    west_rate = round(joined[joined["home_conference"] == "West"]["home_win"].mean() * 100, 1)
